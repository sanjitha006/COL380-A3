...
// --- SEQUENTIAL OPTIMIZATION: Global Efficiency Lookup Arrays ---
// These eliminate the need to ever call std::sort() inside knapsackBound
...
// --- SEQUENTIAL OPTIMIZATION: Zero-Allocation O(N) Knapsack Bound ---
static int knapsackBound(const vector<int>& cand, int rem) {
    if (rem <= 0 || cand.empty()) return 0;

    // 1. Mark current candidates in our fast O(1) lookup array
    ...
    // 2. Iterate through the pre-sorted global array instead of sorting 'cand'
    for (int v : global_eff_order) {
        if (!in_cand[v]) continue; // Skip vertices not in this specific subproblem

        if (cost[v] <= left) {
            ub += profit[v];
            left -= cost[v];
        } else {
            // Fractional upper bound (matches original logic exactly)
            ub += (profit[v] * left) / cost[v]; 
            break;
        }
        
        // Early exit: Stop once we've processed all our candidates
        if (++found == target) break;
    }

    // 3. Reset the lookup array (Faster than allocating a new vector)
...
int main(int argc, char** argv) {
    ...
    // --- SEQUENTIAL OPTIMIZATION: Setup Global Knapsack Lookup ---
    in_cand.assign(N, 0);
    global_eff_order.resize(N);
    iota(global_eff_order.begin(), global_eff_order.end(), 0);
    
    // Sort globally exactly once, using long long to prevent strict overflow
    sort(global_eff_order.begin(), global_eff_order.end(), [&](int a, int b) {
        ...
        if (profit[a] != profit[b]) return profit[a] > profit[b];
        return a < b; // Absolute deterministic tie-breaker for identical graphs
    });
    // -------------------------------------------------------------
