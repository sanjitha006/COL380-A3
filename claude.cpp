 ...
 * Run     : mpirun -n <P> ./clique_mpi < input.txt
 */
...
//  MASTER — rank 0
// ═══════════════════════════════════════════════════════════════════════════
static void runMaster(int P) {
    const int W = P - 1;                    // number of worker ranks

    // ── Generate initial work pool ────────────────────────────────────────
    // Aim for 4× oversubscription so workers have enough variety to keep busy
    // even with unequal subproblem sizes.
    ...
    for (int r = 1; r <= W; r++) {
        if (!Q.empty()) {
            ...
            // No initial work: tell worker to terminate immediately
            int d = 0;
    ...
    // ── Coordination loop ─────────────────────────────────────────────────
    // 16 MB receive buffer — large enough for any serialised subproblem batch.
    const int RBUF_SZ = 1 << 24;
    vector<int> rbuf(RBUF_SZ);

    while (done_count < W) {
        MPI_Status st;
        MPI_Recv(rbuf.data(), RBUF_SZ, MPI_INT,
        ...
        if (tag == TAG_WORK_REQ) {
            if (!Q.empty()) {
                ...
                // No work; mark this worker idle
                is_idle[src] = true;
                idle_count++;

                // Active workers = total workers minus those already terminated
                int active = W - done_count;
                if (idle_count == active) {
                    // All remaining workers are idle AND master queue is empty
                    // → send termination signal to every idle worker
                    for (int r = 1; r <= W; r++) {
        ...
        // ── Worker found a better solution ────────────────────────────────
        // Rebroadcast immediately to all other active workers so they can
        // prune more aggressively right away — this is the key speedup lever.
        ...
        // ── Worker finished and reports its local best ────────────────────
...
int main(int argc, char** argv) {
    ...
    // This gives the bounding functions a non-trivial lower bound from the
    // start, enabling pruning immediately during root decomposition.
    for (int v = 0; v < N; v++) {
