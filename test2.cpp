 ...
 * ─────────────
 * • Optimized for N=1200: Maintains candidate arrays in strict descending 
 * profit order natively to avoid O(V log V) sorting overhead at each tree node.
 * • O(N) Knapsack Bound: Uses a globally pre-sorted efficiency array to eliminate
 * sorting inside the bounding function.
 * • Master (Rank 0): Manages a dynamic task queue of subproblems.
 * • Workers (Ranks 1..P-1): Pull work, run the optimized sequential BnB 
 * engine, and report back tight bounds to prune the search space.
 *
...
static vector<int> global_eff_order;  // Pre-sorted global array for fast knapsack
...
static int knapsackBound(const vector<int>& cand, int rem) {
    ...
    // Fast O(1) lookup to check if a global vertex is in the current candidates
    vector<uint8_t> in_cand(N, 0);
    for (int v : cand) {
        in_cand[v] = 1;
    }
    ...
    int ub = 0, left = rem;
    int items_found = 0;
    int target_items = cand.size();
    
    // Iterate through the pre-sorted global efficiency list
    for (int v : global_eff_order) {
        ...
        // Early exit: Stop looping once we've evaluated all candidates
        items_found++;
        if (items_found == target_items) break;
    }
    return ub;
}
...
int main(int argc, char** argv) {
    ...
    // ── Generate the global efficiency array exactly ONCE for all ranks ──
